    ...
    >
      ...
      <div className={`relative aspect-[3/4] w-full overflow-hidden bg-neutral-950 shrink-0`}>
        {item.img ? (
          ...
          <div className="flex h-full items-center justify-center text-neutral-800 text-xs uppercase">
            NO IMAGE
          </div>
        )}
        ...
        {item.rank > 0 && (
          <div
            className={`absolute top-2 left-2 px-2.5 py-1 rounded-md backdrop-blur-md border text-sm font-mono font-bold shadow-md z-10 ${
              item.rank <= 100
        ...
        {isAdmin && (
          <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black via-black/90 to-transparent z-20 flex items-center justify-between gap-3">
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onUpdateStatus(item.id, status === "ignored" ? null : "ignored");
              }}
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onUpdateStatus(item.id, status === "wishlist" ? null : "wishlist");
              }}
            ...
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                onUpdateStatus(item.id, status === "collected" ? null : "collected");
              }}
        ...
        {status && (
          <div
            className={`absolute top-2 right-2 p-1.5 rounded-full border shadow-md z-10 ${
              status === "collected"
        ...
        {/* Type Badge */}
        <div
          className={`absolute bottom-1 right-1 px-1.5 py-0.5 bg-black/80 backdrop-blur rounded text-[10px] font-bold text-neutral-300 uppercase tracking-wider border border-white/10 transition-opacity ${isAdmin ? "opacity-0" : ""}`}
        >
          {item.type}
        </div>
      </div>
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="min-w-0">
          <h3
            className={`text-sm font-bold leading-snug line-clamp-1 transition-colors ${status === "ignored" ? "text-neutral-500" : "text-neutral-100"}`}
            title={item.cn || item.name}
          >
            {item.cn || item.name}
          </h3>
        ...
        <div className="flex items-end justify-between mt-auto">
          <div className="flex flex-col">
            <span className="text-[9px] text-neutral-600 font-bold uppercase tracking-wider mb-0.5">
              Score
            </span>
            <span
              className={`text-2xl font-bold font-mono leading-none tracking-tight ${status === "ignored" ? "text-neutral-600" : scoreColor}`}
            >
