...
import type { ExplorerFilters, SortKey, StatusFilterType } from "@/features/explorer/state";

export type { StatusFilterType };

type FilterKey = keyof ExplorerFilters;
type PresetFilters = Pick<ExplorerFilters, "year" | "votes" | "rank" | "score">;

interface FilterPanelProps {
  filters: ExplorerFilters;
  setFilter: (key: FilterKey, val: [number, number]) => void;
  applyFilters: (filters: Partial<ExplorerFilters>, types?: Set<string>) => void;
  resetAll: () => void;
...
export function FilterPanel({
  ...
  }, [selectedTypes, useShortSeries, useSeriesOnly, filters.eps, setFilter]);

  const updateMin = (key: FilterKey, val: number, limitMin: number, limitMax: number) => {
  ...
  const updateMax = (key: FilterKey, val: number, limitMin: number, limitMax: number) => {
    let v = Math.max(limitMin, Math.min(val, limitMax));
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex items-center gap-6 shrink-0 w-full md:w-auto justify-between md:justify-start">
            <div className="flex flex-col select-none">
              ...
              <div className="text-[9px] text-neutral-500 font-mono font-bold uppercase tracking-widest flex items-center gap-2">
                ...
                <span className={showingCount === 0 ? "text-red-500" : "text-neutral-400"}>
                  {showingCount.toLocaleString()} Found
                </span>
              </div>
            </div>
          ...
          <div className="relative w-full max-w-xl group hidden md:block">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <Search
                className="text-neutral-600 group-focus-within:text-pink-500 transition-colors"
                size={16}
              />
            </div>
            <input
                ...
                <div className="space-y-6 pb-20 md:pb-0">
                  ...
                  <div className="flex flex-col xl:flex-row items-center justify-between gap-4 pb-4 border-b border-neutral-800/50">
                    <div className="grid grid-cols-2 gap-2 w-full md:w-auto md:flex md:flex-wrap md:justify-start">
                      <div className="col-span-2 md:w-auto flex justify-center md:justify-start">
                        <span className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider py-1.5 mr-2">
                          Presets
                        </span>
                      </div>
                      {PRESETS.map((preset) => {
                    ...
                    <div
                      className={`grid grid-cols-2 gap-2 w-full md:w-auto md:flex md:items-center transition-opacity duration-300 ${isMovieSelected ? "opacity-30 pointer-events-none" : "opacity-100"}`}
                    >
                      <button
                        type="button"
                      ...
                      >
                        <Zap
                          size={12}
                          className={useShortSeries ? "text-indigo-400" : "text-neutral-600"}
                        />
                        Max 52 Eps
                      </button>
                      <button
                      ...
                      >
                        <Layers
                          size={12}
                          className={useSeriesOnly ? "text-teal-400" : "text-neutral-600"}
                        />
                        {">"} 1 Ep
                      </button>
                    </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                    <FilterInputGroup
                      ...
                      limitMin={0}
                      limitMax={2030}
                      onMin={(v) => updateMin("year", v, 0, 2030)}
                      onMax={(v) => updateMax("year", v, 0, 2030)}
                    ...
                    <FilterInputGroup
                      ...
                      limitMin={0}
                      limitMax={10}
                      onMin={(v) => updateMin("score", v, 0, 10)}
                      onMax={(v) => updateMax("score", v, 0, 10)}
                    />
                    <FilterInputGroup
                      icon={<Trophy size={14} />}
                      label="Rank"
                      ...
                      limitMin={0}
                      limitMax={99999}
                      onMin={(v) => updateMin("rank", v, 0, 99999)}
                      onMax={(v) => updateMax("rank", v, 0, 99999)}
                    />
                    <FilterInputGroup
                      icon={<Users size={14} />}
                      label="Votes"
                      ...
                      limitMin={0}
                      limitMax={999999}
                      onMin={(v) => updateMin("votes", v, 0, 999999)}
                      onMax={(v) => updateMax("votes", v, 0, 999999)}
                    />
                    <FilterInputGroup
                      icon={<Layers size={14} />}
                      label="Episodes"
                      ...
                      limitMin={0}
                      limitMax={9999}
                      onMin={(v) => updateMin("eps", v, 0, 9999)}
                      onMax={(v) => updateMax("eps", v, 0, 9999)}
                    />
                  </div>
                  ...
                  <div className="flex flex-col xl:flex-row gap-8 pt-4 border-t border-neutral-800/50">
                    ...
                    <div className="xl:ml-auto flex items-end gap-3">
                      ...
                      <button
                        type="button"
                        onClick={resetAll}
                        className="px-6 py-2 rounded-lg text-[11px] font-bold border border-transparent text-neutral-500 hover:text-red-400 hover:bg-red-900/10 transition-all flex items-center gap-2"
                      >
...
function FilterInputGroup({
  ...
  return (
    <div
      className={`relative flex flex-col gap-2 p-3 rounded-xl border transition-all duration-300 group ${active ? "bg-pink-900/5 border-pink-500/30" : "bg-neutral-900/30 border-neutral-800 hover:border-neutral-700"}`}
    >
      <div
        className={`flex items-center gap-2 transition-colors ${active ? "text-pink-400" : "text-neutral-500 group-hover:text-neutral-400"}`}
      >
        {icon} <span className="text-[10px] font-bold uppercase tracking-wider">{label}</span>
      </div>
      <div className="flex items-center gap-2">
        <Input val={min} onChange={onMin} step={step} min={limitMin} max={limitMax} />
        <span className="text-neutral-700 font-light">/</span>
        <Input val={max} onChange={onMax} step={step} min={limitMin} max={limitMax} />
      </div>
      {active && (
        <div className="absolute inset-0 border border-pink-500/20 rounded-xl pointer-events-none animate-pulse" />
      )}
    </div>
  );
}
...
function Input({
  ...
  const commit = () => {
    let num = Number.parseFloat(localVal);
