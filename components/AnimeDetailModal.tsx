...
import type { ItemStatus } from "@/features/collection/state";
...
const getSiteUrl = (site: string, id: string) => {
  switch (site) {
    ...
    case "iqiyi":
      return `https://www.iqiyi.com/${id}.html`;
    case "qq":
      return `https://v.qq.com/x/cover/${id}.html`;
    case "nicovideo":
      return `https://ch.nicovideo.jp/${id}`;
    case "netflix":
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center md:p-6">
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 bg-black/50 text-white rounded-full hover:bg-neutral-700 transition-colors backdrop-blur-md"
        >
          <X size={20} />
        </button>
        ...
        <div className="w-full md:w-[300px] bg-neutral-950 p-5 flex flex-col gap-5 border-b md:border-b-0 md:border-r border-neutral-800 overflow-y-auto shrink-0 custom-scrollbar">
          ...
          {!isAdmin && status && (
            <div
              className={`w-full py-3 rounded-md font-bold text-sm tracking-wide flex items-center justify-center gap-2 select-none cursor-default border ${
                status === "collected"
          ...
          <div className="bg-neutral-900 rounded-lg p-3 border border-neutral-800">
            <h4 className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-3">
              Community Stats
            </h4>
            <div className="space-y-3">
              <StatRow
                icon={<Eye size={14} />}
                label="Watched"
                value={item.collection?.collect}
                color="text-blue-400"
              />
              <StatRow
                icon={<Heart size={14} />}
                label="Wish"
                value={item.collection?.wish}
                color="text-pink-400"
              />
              <StatRow
                icon={<Clock size={14} />}
                label="Doing"
                value={item.collection?.doing}
                color="text-yellow-400"
              />
              <StatRow
                icon={<PauseCircle size={14} />}
                label="On Hold"
                value={item.collection?.on_hold}
        ...
        <div className="flex-1 overflow-y-auto bg-neutral-900/50 custom-scrollbar">
          <div className="p-6 md:p-10 space-y-8 pb-24 md:pb-10">
            <div className="hidden md:block">
              ...
              {item.cn !== item.name && (
                <h2 className="text-lg md:text-xl text-neutral-500 font-medium">{item.name}</h2>
              )}
            </div>
            ...
            <div className="bg-black/20 p-4 md:p-6 rounded-xl border border-neutral-800/50 flex flex-col md:flex-row gap-8">
              ...
              <div className="flex flex-row md:flex-col items-center md:items-start justify-between md:justify-center min-w-[120px] w-full md:w-auto gap-4">
                <div className="flex flex-col items-center md:items-start">
                  <div className="text-4xl md:text-5xl font-bold text-yellow-400 font-mono tracking-tighter">
                    {item.score.toFixed(1)}
                  </div>
                  <div className="text-[10px] md:text-xs text-neutral-500 mt-1 font-mono">
                    {item.total.toLocaleString()} votes
                  </div>
                </div>
                <div className="mt-0 md:mt-4 px-3 py-1 bg-neutral-800 rounded text-xs text-neutral-300 font-mono text-center border border-neutral-700">
                  Rank #{item.rank}
                </div>
              </div>
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 bg-neutral-800 border border-neutral-700/50 rounded text-xs text-neutral-400 cursor-default hover:text-white transition-colors"
                      >
                        {tag}
                      </span>
                    ))}
...
function Badge({
  ...
  return (
    <div
      className={`px-2 py-1 rounded text-[10px] font-bold tracking-wider uppercase flex items-center gap-1.5 border border-transparent ${color}`}
    >
      {icon} {text}
    </div>
  );
